...
function hitoast(text, options) {
	var defaultOptions = {
		...
		'container': 'body' // Id of the alert container.
	};
	...
	var html = $('<div class="hiToast" style="max-width: 60%;padding: 5px 15px;z-index: 999;border-radius: 4px;position: fixed;background-color: rgba(0,0,0,.7);color: #fff;line-height: 1.4em;font-size: 12px;">' + text + '</div>').fadeIn(options['fade-time']);

	$container.append(html);

	var width = ($(window).width() / 2 - $(".hiToast").outerWidth() / 2) + "px";
	var height = '20px';
	if (options.position == 'top') {
		height = ($(window).height() * 1 / 5 - $(".hiToast").outerHeight() / 2) + "px";
	} else if (options.position == 'center') {
		height = ($(window).height() / 2 - $(".hiToast").outerHeight() / 2) + "px";
	} else if (options.position == 'bottom') {
		height = ($(window).height() * 4 / 5 - $(".hiToast").outerHeight() / 2) + "px";
	}

	$(".hiToast").css("left", width);
	$(".hiToast").css("top", height);
...
function hiToastX(element, options) {
	...
	if (typeof element !== "undefined") {
		...
		$('.hiToast').fadeOut(options.duration, function() {
			$(this).remove();
		});
	}
}
